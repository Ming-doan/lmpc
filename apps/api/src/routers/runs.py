...
async def list_runs(
    ...
    """List public benchmark runs with cursor-based pagination"""
...
async def create_run(
    ...
    """Create a new benchmark run and enqueue it for processing. Admin-only."""
    # Create run record in DB with "queued" status
    run = BenchmarkRun(
    ...
    # Add run to processing queue in Redis
...
async def get_run(
    ...
    """Get details of a benchmark run by ID"""
...
async def cancel_run(
    ...
    """Cancel a benchmark run by ID. Admin-only."""
    r = await get_redis()
...
async def run_events(
    ...
    """Stream real-time events for a benchmark run by ID"""
